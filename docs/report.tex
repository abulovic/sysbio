...
First, we tested the simple rejection sampler(\ref{sec:rejection_sampler}). The priors for model parameters $a$ and $b$ were chosen to be uniform $(a,b) \sim \mathcal{U}(-5, 5)$.
\subsubsection{Parameter inference for repressilator model}
To test the implemented SMC algorithm we turn into a molecular biology example, the repressilator model which was purposefully chosen to highlight some of the challenges associated with parameter estimation and also the utility of the information gathered from ABC algorithms. The repressilator model is a popular synthetic gene regulatory network. It consists of three genes connected in a feedback loop and each of the genes transcribes for a protein that represses the next gene in the loop with the last one repressing the first. 
\begin{figure}
\centering
\includegraphics[width=0.5\linewidth]{ABCSMC}
